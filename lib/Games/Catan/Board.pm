...
sub BUILD {
    ...
    my $generic_harbor = Games::Catan::Harbor->new(
        brick_ratio  => 3,
        lumber_ratio => 3,
        wool_ratio   => 3,
        grain_ratio  => 3,
...
sub place_settlement {
    ...
    # Is there already a building here?
...
sub upgrade_settlement {
    ...
    # Grab texisting settlement at this location.
    my $settlement = $self->graph->get_vertex_attribute(
        $intersection,
        'building',
    );
...
sub get_valid_settlement_intersections {
    ...
    # Can only place their settlements on intersections that their roads are
    # attached to.
    ...
    # But remove those intersections which violate the distance rule (no
    # settlement can be one hop away from another).
...
sub get_longest_road {
    ...
    for my $path ( @paths ) {
        ...
        # Only start from intersections that have a road built by this player.
        next if !$self->graph->has_edge_attribute( $u, $v, 'road' );
        ...
        # Recurse and track all possible road paths starting from both
        # intersections.
        $self->_traverse_roads(
...
sub _traverse_roads {
    ...
    for my $adjacent ( @adjacent_paths ) {
        my ( $u2, $v2 ) = @$adjacent;
        ...
        my $adj_road = $self->graph->get_edge_attribute( $u2, $v2, 'road' );

        next if $adj_road->player->color ne $player->color;

        # Which intersection is connecting this next path (which direction are
        # we going to)?
        my $intersection = $self->_get_connecting_intersection(
            $current_path,
            $adjacent,
        );
        ...
        if ( $self->graph->has_vertex_attribute( $intersection, 'building' ) ) {
            my $building = $self->graph->get_vertex_attribute(
                $intersection,
                'building',
            );
        ...
        # Make sure we mark the correct order of the intersection we're
        # traversing.
